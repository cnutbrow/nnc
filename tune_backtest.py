...
def _run_precomputed(engine, sym_data: dict) -> dict:
    """Like engine.run() but skips inference."""
...
def main():
    ...
    # ── Parameter grid (probability thresholds) ───────────────────────────────
    long_thresholds = [0.52, 0.54, 0.55, 0.57, 0.60, 0.62]
    max_pos_pcts    = [0.04, 0.06, 0.08, 0.12]
    sl_tp_pairs     = [(0.02, 0.04), (0.03, 0.06), (0.03, 0.09), (0.04, 0.08)]
    allow_shorts    = [True, False]

    grid = list(itertools.product(long_thresholds, max_pos_pcts, sl_tp_pairs, allow_shorts))
    print(f'Sweeping {len(grid)} parameter combinations…\n')

    results = []

    for long_thr, max_pos, (sl, tp), shorts in grid:
        cfg = copy.copy(base_cfg)
        cfg.long_entry_threshold  = long_thr
        cfg.short_entry_threshold = 1.0 - long_thr
    ...
    print(f'{"LongThr":>7} {"MaxPos":>6} {"SL":>4} {"TP":>4} {"Shorts":>6} '
    ...
    for r in results[:25]:
        print(
            f'{r["long_thr"]:>7.2f} {r["max_pos"]:>6.2f} {r["sl"]:>4.2f} {r["tp"]:>4.2f} '
    ...
    print('\n── Top config ───────────────────────────────────────────────────────')
    best = results[0]
    print(f'  long_entry_threshold  = {best["long_thr"]}')
    print(f'  short_entry_threshold = {1.0 - best["long_thr"]}')
